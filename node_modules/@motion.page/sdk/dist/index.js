function u(J,$){switch(J){case"play":case"resume":$.play();break;case"pause":$.pause();break;case"reverse":$.reverse();break;case"restart":$.restart();break;case"reset":$.pause(),$.progress(0);break;case"complete":$.progress(1),$.pause();break;case"none":break;default:console.warn(`[Motion] Unknown timeline action: "${J}". Valid actions: play, pause, resume, reverse, restart, reset, complete, none`)}}var O={color:null,filter:null,drawSVG:null,clipPath:null,stagger:null,textSplitter:null,textFlapper:null,triggerManager:null,styleReset:null,fit:null,responsive:null};class D{static _instance;static _triggerRegistry=new Map;static _pageLoadTriggerFactory=null;static _RESIZE_DEBOUNCE_MS=200;static _BODY_RESIZE_DEBOUNCE_MS=250;_pageLoadTrigger=null;_scrollTriggers=new Map;_eventTriggers=new Map;_mouseMoveTriggers=new Map;_gestureTriggers=new Map;_cursorTriggers=new Map;_pageExitTriggers=new Map;_resizeHandler=null;_resizeTimer=null;_bodyResizeObserver=null;_lastBodyHeight=0;_bodyResizeTimer=null;get _allTriggerMaps(){return[this._scrollTriggers,this._eventTriggers,this._mouseMoveTriggers,this._gestureTriggers,this._cursorTriggers,this._pageExitTriggers]}constructor(){}static getInstance(){if(!D._instance)D._instance=new D;return D._instance}static registerTrigger(J,$){D._triggerRegistry.set(J,$)}static registerPageLoadTrigger(J){D._pageLoadTriggerFactory=J}_getPageLoadTrigger(){if(!this._pageLoadTrigger&&D._pageLoadTriggerFactory)this._pageLoadTrigger=new D._pageLoadTriggerFactory;return this._pageLoadTrigger}_createTrigger(J,...$){let Q=D._triggerRegistry.get(J);if(!Q){let Z=Array.from(D._triggerRegistry.keys());return console.warn(`[Motion SDK] Trigger type '${J}' is not available in this bundle. Available triggers: ${Z.join(", ")||"none"}. Animations using .on${J.charAt(0).toUpperCase()+J.slice(1)}() will be skipped.`),null}return new Q(...$)}registerPageLoad(J,$){this.unregister(J);let Q=this._getPageLoadTrigger();if(Q)Q.register(J,$)}_attachResizeListener(){if(this._resizeHandler||typeof window>"u")return;this._resizeHandler=()=>{if(this._resizeTimer!==null)clearTimeout(this._resizeTimer);this._resizeTimer=setTimeout(()=>{this._resizeTimer=null,this.refreshScrollTriggers()},D._RESIZE_DEBOUNCE_MS)},window.addEventListener("resize",this._resizeHandler,{passive:!0})}_detachResizeListener(){if(!this._resizeHandler||typeof window>"u")return;if(window.removeEventListener("resize",this._resizeHandler),this._resizeTimer!==null)clearTimeout(this._resizeTimer),this._resizeTimer=null;this._resizeHandler=null}_attachBodyResizeObserver(){if(this._bodyResizeObserver)return;if(typeof window>"u"||typeof document>"u")return;if(typeof ResizeObserver>"u")return;this._lastBodyHeight=document.body.offsetHeight,this._bodyResizeObserver=new ResizeObserver((J)=>{for(let $ of J){let Q=$.borderBoxSize?.[0]?$.borderBoxSize[0].blockSize:$.contentRect.height;if(Math.abs(Q-this._lastBodyHeight)<1)continue;if(this._lastBodyHeight=Q,this._bodyResizeTimer!==null)clearTimeout(this._bodyResizeTimer);this._bodyResizeTimer=setTimeout(()=>{this._bodyResizeTimer=null,this.refreshScrollTriggers()},D._BODY_RESIZE_DEBOUNCE_MS)}}),this._bodyResizeObserver.observe(document.body)}_detachBodyResizeObserver(){if(!this._bodyResizeObserver)return;if(this._bodyResizeObserver.disconnect(),this._bodyResizeObserver=null,this._bodyResizeTimer!==null)clearTimeout(this._bodyResizeTimer),this._bodyResizeTimer=null;this._lastBodyHeight=0}registerScroll(J,$){this.unregister(J);let Q=this._createTrigger("scroll",J,$);if(!Q)return;this._scrollTriggers.set(J,Q),J._trigger=Q,Q.enable(),this._attachResizeListener(),this._attachBodyResizeObserver()}registerHover(J,$){this.unregister(J);let Q=this._createTrigger("event",J,{type:"hover",target:$.target,onEnter:$.onEnter,onLeave:$.onLeave,leaveDelay:$.leaveDelay});if(!Q)return;this._eventTriggers.set(J,Q),J._trigger=Q,Q.enable()}registerClick(J,$){this.unregister(J);let Q=this._createTrigger("event",J,{type:"click",target:$.target,secondTarget:$.secondTarget,toggle:$.toggle,preventDefault:$.preventDefault});if(!Q)return;this._eventTriggers.set(J,Q),J._trigger=Q,Q.enable()}registerMouseMove(J,$){this.unregister(J);let Q=this._createTrigger("mouseMove",J,$);if(!Q)return;this._mouseMoveTriggers.set(J,Q),J._trigger=Q,Q.enable()}registerGesture(J,$){this.unregister(J);let Q=this._createTrigger("gesture",J,$);if(!Q)return;this._gestureTriggers.set(J,Q),J._trigger=Q,Q.enable()}registerCursor(J,$){this.unregister(J);let Q=this._createTrigger("cursor",J,$);if(!Q)return;this._cursorTriggers.set(J,Q),J._trigger=Q,Q.enable()}registerPageExit(J,$){this.unregister(J);let Q=this._createTrigger("pageExit",J,$);if(!Q)return;this._pageExitTriggers.set(J,Q),J._trigger=Q,Q.enable()}unregister(J){let $=this._getPageLoadTrigger();if($)$.unregister(J);for(let Q of this._allTriggerMaps){let Z=Q.get(J);if(Z)Z.disable(),Q.delete(J)}if(J._trigger=void 0,this._scrollTriggers.size===0)this._detachResizeListener(),this._detachBodyResizeObserver()}refreshScrollTriggers(){let J=Array.from(this._scrollTriggers.values()),$=typeof window<"u"?window.scrollY:0;J.sort((Q,Z)=>{let j=Q._pinManager?.getLayoutRect?.(),q=Z._pinManager?.getLayoutRect?.();if(!j&&!q)return 0;if(!j)return 1;if(!q)return-1;return j.top+$-(q.top+$)}),J.forEach((Q)=>Q.refresh?.()),this._eventTriggers.forEach((Q)=>Q.refresh?.()),this._gestureTriggers.forEach((Q)=>Q.refresh?.()),this._mouseMoveTriggers.forEach((Q)=>Q.refresh?.())}killAll(){let J=this._getPageLoadTrigger();if(J)J.clear();for(let $ of this._allTriggerMaps)$.forEach((Q)=>Q.disable()),$.clear();this._detachResizeListener(),this._detachBodyResizeObserver()}static _reset(){if(D._instance){if(D._instance._pageLoadTrigger)D._instance._pageLoadTrigger._reset();D._instance.killAll(),D._instance=void 0}}}O.triggerManager={getInstance:D.getInstance};function W6(J){if(typeof document>"u"){J();return}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",J,{once:!0});else J()}function U6(J){if(J===null||typeof J!=="object")return!1;if(J instanceof Element)return!1;if("nodeType"in J)return!1;if("style"in J)return!1;let $=J;if("x"in $&&typeof $.x==="number")return!0;if("length"in J)return!1;return!0}function w0(J){return J instanceof Element||typeof J==="object"&&J!==null&&(("nodeType"in J)||("style"in J))}function zJ(J){if(U6(J))return[J];if(typeof J==="string"){if(typeof document<"u")try{return Array.from(document.querySelectorAll(J))}catch($){return console.warn(`[Motion] Invalid selector "${J}":`,$),[]}return[]}if(J instanceof Element)return[J];if(J&&typeof J==="object"&&"style"in J&&!Array.isArray(J))return[J];if(J instanceof NodeList)return Array.from(J);if(Array.isArray(J)){if(J.length===0)return[];if(typeof J[0]==="string"){if(typeof document>"u")return[];let $=[];for(let Q of J)try{$.push(...Array.from(document.querySelectorAll(Q)))}catch(Z){console.warn(`[Motion] Invalid selector "${Q}":`,Z)}return $}if(U6(J[0]))return J;return J}if(J&&typeof J==="object"&&"length"in J)return Array.from(J);return[]}class g{_timeline;_config;_enabled=!1;constructor(J,$){this._timeline=J,this._config=$}enable(){if(this._enabled)return;this._enabled=!0,W6(()=>{if(!this._enabled)return;this._onEnable()})}disable(){if(!this._enabled)return;this._enabled=!1,this._onDisable()}_resolveElement(J,$){if(J===void 0||J===null)return $??null;if(typeof J!=="string")return J;if(typeof document<"u")try{let Q=document.querySelector(J);if(Q!==null)return Q}catch(Q){console.warn(`[Motion] Invalid selector "${J}":`,Q)}return $??null}_resolveElements(J){if(J===void 0||J===null)return[];if(typeof J!=="string")return[J];if(typeof document<"u")try{let $=document.querySelectorAll(J);return Array.from($)}catch($){return console.warn(`[Motion] Invalid selector "${J}":`,$),[]}return[]}_cleanupListenerMap(J){J.forEach(($,Q)=>{$.forEach((Z,j)=>{Q.removeEventListener(j,Z)})}),J.clear()}}function n(J){let $=J,Q=$.style.transform,Z=$.style.transition;$.style.transition="none",$.style.transform="none";let j=!1,q=$.style.position,z=$.style.top,G=$.style.left,N=$.style.width;if(q==="fixed")j=!0;else if(q===""||q===void 0){if(typeof window<"u"&&typeof window.getComputedStyle==="function"&&window.getComputedStyle($).position==="fixed")j=!0}if(j)$.style.position="",$.style.top="",$.style.left="",$.style.width="";let H=$.getBoundingClientRect();if($.style.transform=Q,$.style.transition=Z,j)$.style.position=q,$.style.top=z,$.style.left=G,$.style.width=N;return H}var i0=new Map;function O6(){return typeof window<"u"?window:null}function x7(J,$){let Q=typeof window<"u"?window.requestAnimationFrame:void 0;if(typeof Q!=="function"){$();return}if(J.rafId!==null)return;J.rafId=Q(()=>{J.rafId=null,$()})}function Q0(J,$=O6()){if(!$)return;let Q=i0.get($);if(!Q)Q={callbacks:new Set,lastEvent:null,rafId:null,listener:(Z)=>{let j=i0.get($);if(!j||j.callbacks.size===0)return;j.lastEvent=Z,x7(j,()=>{let q=j.lastEvent;if(!q)return;let z=Array.from(j.callbacks);for(let G of z)try{G(q)}catch(N){console.error("[Motion] Uncaught error in pointer callback:",N)}})}},i0.set($,Q),$.addEventListener("mousemove",Q.listener,{passive:!0});Q.callbacks.add(J)}function X0(J,$=O6()){if(!$)return;let Q=i0.get($);if(!Q)return;if(Q.callbacks.delete(J),Q.callbacks.size===0){if(Q.rafId!==null&&typeof window<"u"&&typeof window.cancelAnimationFrame==="function")window.cancelAnimationFrame(Q.rafId);$.removeEventListener("mousemove",Q.listener),i0.delete($)}}class I6 extends g{_targets=[];_secondTargets=[];_listeners=new Map;_isForward=!0;_frozenRects=new Map;_snapshotScrollX=0;_snapshotScrollY=0;_wasHovering=!1;_hoverLeaveTimeout=null;_boundPointerMoveHandler=null;constructor(J,$){super(J,$)}_onEnable(){if(this._resolveTargets(),this._config.type==="hover")this._addHoverListeners();else if(this._config.type==="click")this._addClickListeners()}_onDisable(){if(this._boundPointerMoveHandler)X0(this._boundPointerMoveHandler),this._boundPointerMoveHandler=null;if(this._hoverLeaveTimeout!==null)clearTimeout(this._hoverLeaveTimeout),this._hoverLeaveTimeout=null;this._frozenRects.clear(),this._wasHovering=!1,this._cleanupListenerMap(this._listeners)}refresh(){if(this._config.type!=="hover")return;this._snapshotRects()}_snapshotRects(){if(typeof window>"u")return;this._snapshotScrollX=window.scrollX||0,this._snapshotScrollY=window.scrollY||0;for(let J of this._targets)this._frozenRects.set(J,n(J))}_isInsideFrozenRects(J,$){if(typeof window>"u")return!1;let Q=(window.scrollX||0)-this._snapshotScrollX,Z=(window.scrollY||0)-this._snapshotScrollY,j=J+Q,q=$+Z;for(let z of this._frozenRects.values())if(j>=z.left&&j<=z.right&&q>=z.top&&q<=z.bottom)return!0;return!1}_isOverLiveTarget(J,$){if(typeof document>"u"||typeof document.elementsFromPoint!=="function")return null;let Q=document.elementsFromPoint(J,$);for(let Z of Q)for(let j of this._targets)if(Z===j||typeof j.contains==="function"&&j.contains(Z))return!0;return!1}_resolveTargets(){if(this._targets=this._resolveElements(this._config.target),this._config.secondTarget)this._secondTargets=this._resolveElements(this._config.secondTarget);else this._secondTargets=this._targets}_addHoverListeners(){let J=this._config.onEnter||"play",$=this._config.onLeave||"reverse",Q=this._config.leaveDelay||0;this._snapshotRects(),this._boundPointerMoveHandler=(Z)=>{let j;if(this._wasHovering)j=this._isInsideFrozenRects(Z.clientX,Z.clientY);else j=this._isOverLiveTarget(Z.clientX,Z.clientY)??this._isInsideFrozenRects(Z.clientX,Z.clientY);if(j&&!this._wasHovering){if(this._wasHovering=!0,this._snapshotRects(),this._hoverLeaveTimeout!==null)clearTimeout(this._hoverLeaveTimeout),this._hoverLeaveTimeout=null;if(J==="restart"){let q=this._timeline.progress();if(q>=1||q===0)this._timeline.restart()}else this._timeline.play()}else if(!j&&this._wasHovering){if(this._wasHovering=!1,$==="none")return;let q=()=>{this._handleLeaveAction($),this._hoverLeaveTimeout=null};if(Q>0)this._hoverLeaveTimeout=window.setTimeout(q,Q*1000);else q()}},Q0(this._boundPointerMoveHandler)}_handleLeaveAction(J){u(J==="stop"?"reset":J,this._timeline)}_addClickListeners(){let J=this._config.toggle||"play",$=this._config.preventDefault||!1,Q=this._config.secondTarget&&this._config.secondTarget!==this._config.target;if(this._targets.forEach((Z)=>{let j=this._listeners.get(Z)||new Map,q=(z)=>{if($)z.preventDefault();if(!Q)if(this._isForward){if(this._timeline.restart(),J!=="play")this._isForward=!1}else this._handleSecondClick(J),this._isForward=!0;else this._timeline.restart()};Z.addEventListener("click",q),j.set("click",q),this._listeners.set(Z,j)}),Q)this._secondTargets.forEach((Z)=>{if(this._listeners.has(Z))return;let j=new Map,q=(z)=>{if($)z.preventDefault();this._handleSecondClick(J)};Z.addEventListener("click",q),j.set("click",q),this._listeners.set(Z,j)})}_handleSecondClick(J){u(J,this._timeline)}}D.registerTrigger("event",I6);var HJ={perspective:0,x:1,y:2,z:3,rotate:4,rotateX:5,rotateY:6,rotateZ:7,scale:8,scaleX:9,scaleY:10,scaleZ:11,skewX:12,skewY:13},uJ=["px","px","px","px","deg","deg","deg","deg","","","","","deg","deg"],S0=new WeakMap,NJ=new WeakMap,x=[];function Z0(J){let $=J.trim().match(/([-+]?\d*\.?\d+(?:e[-+]?\d+)?)([a-z%]*)/i);if(!$)return{value:0,unit:""};return{value:parseFloat($[1]),unit:$[2]||""}}function C0(J,$){switch($){case"rad":return J*180/Math.PI;case"turn":return J*360;case"grad":return J*0.9;default:return J}}function U0(J){let $=Z0(J);return C0($.value,$.unit||"deg")}function M(J,$,Q,Z){let j=HJ[$];if(j===void 0)return;J.values[j]=Q;let q=Z&&Z.length>0?Z:uJ[j]||"";J.units[j]=q,J.used|=1<<j}function w7(J,$){let Q=/([a-zA-Z0-9]+)\(([^)]+)\)/g,Z;while((Z=Q.exec(J))!==null){let j=Z[1],q=Z[2].match(/[-+]?\d*\.?\d+(?:e[-+]?\d+)?[a-z%]*/gi)||[];switch(j){case"translate":{let z=q[0]?Z0(q[0]):{value:0,unit:""},G=q[1]?Z0(q[1]):{value:0,unit:""};M($,"x",z.value,z.unit),M($,"y",G.value,G.unit);break}case"translate3d":{let z=q[0]?Z0(q[0]):{value:0,unit:""},G=q[1]?Z0(q[1]):{value:0,unit:""},N=q[2]?Z0(q[2]):{value:0,unit:""};M($,"x",z.value,z.unit),M($,"y",G.value,G.unit),M($,"z",N.value,N.unit);break}case"translateX":{let z=q[0]?Z0(q[0]):{value:0,unit:""};M($,"x",z.value,z.unit);break}case"translateY":{let z=q[0]?Z0(q[0]):{value:0,unit:""};M($,"y",z.value,z.unit);break}case"translateZ":{let z=q[0]?Z0(q[0]):{value:0,unit:""};M($,"z",z.value,z.unit);break}case"scale":{let z=q[0]?parseFloat(q[0]):1,G=q[1]?parseFloat(q[1]):z;M($,"scaleX",z),M($,"scaleY",G);break}case"scale3d":{let z=q[0]?parseFloat(q[0]):1,G=q[1]?parseFloat(q[1]):1,N=q[2]?parseFloat(q[2]):1;M($,"scaleX",z),M($,"scaleY",G),M($,"scaleZ",N);break}case"scaleX":{let z=q[0]?parseFloat(q[0]):1;M($,"scaleX",z);break}case"scaleY":{let z=q[0]?parseFloat(q[0]):1;M($,"scaleY",z);break}case"scaleZ":{let z=q[0]?parseFloat(q[0]):1;M($,"scaleZ",z);break}case"rotate":M($,"rotate",q[0]?U0(q[0]):0,"deg");break;case"rotateX":M($,"rotateX",q[0]?U0(q[0]):0,"deg");break;case"rotateY":M($,"rotateY",q[0]?U0(q[0]):0,"deg");break;case"rotateZ":M($,"rotateZ",q[0]?U0(q[0]):0,"deg");break;case"skew":{let z=q[0]?U0(q[0]):0,G=q[1]?U0(q[1]):0;M($,"skewX",z,"deg"),M($,"skewY",G,"deg");break}case"skewX":M($,"skewX",q[0]?U0(q[0]):0,"deg");break;case"skewY":M($,"skewY",q[0]?U0(q[0]):0,"deg");break;case"perspective":{let z=q[0]?Z0(q[0]):{value:0,unit:""};M($,"perspective",z.value,z.unit);break}default:break}}}function hJ(J,$,Q,Z,j,q,z,G,N,H,K,Y,_,X){if(_){let i=Math.hypot(J,$),K0=J*j-$*Z,s0=i!==0?K0/i:0,t=i!==0?Math.atan2($,J):0,$0=i!==0?Math.atan2(J*Z+$*j,i*i):0;if(M(X,"x",H,"px"),M(X,"y",K,"px"),M(X,"scaleX",i||1),M(X,"scaleY",s0||1),M(X,"rotate",C0(t,"rad"),"deg"),Math.abs($0)>0.000001)M(X,"skewX",C0($0,"rad"),"deg");return}let U=Math.hypot(J,$,Q)||1,W=Math.hypot(Z,j,q)||1,B=Math.hypot(z,G,N)||1,I=J/U,F=$/U,L=Q/U,A=Z/W,k=j/W,L0=q/W,M0=z/B,l0=G/B,D0=N/B,s=Math.sqrt(I*I+A*A),x0=s<0.000001,R0=0,Y0=0,k0=0;if(!x0)R0=Math.atan2(l0,D0),Y0=Math.atan2(-M0,s),k0=Math.atan2(A,I);else R0=Math.atan2(-L0,k),Y0=Math.atan2(-M0,s),k0=0;M(X,"x",H,"px"),M(X,"y",K,"px"),M(X,"z",Y,"px"),M(X,"scaleX",U),M(X,"scaleY",W),M(X,"scaleZ",B),M(X,"rotateX",C0(R0,"rad"),"deg"),M(X,"rotateY",C0(Y0,"rad"),"deg"),M(X,"rotateZ",C0(k0,"rad"),"deg")}function C7(J,$){if(!J||J==="none")return;if(typeof DOMMatrixReadOnly<"u"){let Z=new DOMMatrixReadOnly(J);hJ(Z.m11,Z.m12,Z.m13,Z.m21,Z.m22,Z.m23,Z.m31,Z.m32,Z.m33,Z.m41,Z.m42,Z.m43,Z.is2D,$);return}let Q=J.match(/[-+]?\d*\.?\d+(?:e[-+]?\d+)?/gi)||[];if(J.startsWith("matrix3d")&&Q.length>=16){let Z=Q.map(Number);hJ(Z[0],Z[1],Z[2],Z[4],Z[5],Z[6],Z[8],Z[9],Z[10],Z[12],Z[13],Z[14],!1,$);return}if(J.startsWith("matrix")&&Q.length>=6){let Z=Q.map(Number);hJ(Z[0],Z[1],0,Z[2],Z[3],0,0,0,1,Z[4],Z[5],0,!0,$)}}function S7(J,$){if(!(J instanceof HTMLElement)&&!(J instanceof SVGElement))return;let Z=J.style.transform;if(Z&&Z!=="none"){w7(Z,$),$.cachedString=Z;return}try{if(typeof window<"u"&&window.getComputedStyle){let j=window.getComputedStyle(J),q=j.transform||j.getPropertyValue("transform");if(q&&q!=="none")C7(q,$),$.cachedString=q}}catch{}}function dJ(J){let $=S0.get(J);if(!$){let Q=new Float32Array(14);Q[8]=1,Q[9]=1,Q[10]=1,Q[11]=1,$={values:Q,units:[...uJ],dirty:0,used:0,cachedString:"",pinDirty:!1},S7(J,$),S0.set(J,$)}return $}function V(J,$,Q,Z){let j=dJ(J),q=HJ[$];if(q!==void 0){if(j.values[q]=Q,Z!==void 0)j.units[q]=Z;if(j.used|=1<<q,$==="scale")j.values[9]=Q,j.values[10]=Q,j.dirty|=1536;else if($==="scaleX"||$==="scaleY")j.values[8]=1,j.dirty|=256;if($==="rotate")j.values[7]=0,j.dirty|=128;else if($==="rotateZ")j.values[4]=0,j.dirty|=16;j.dirty|=1<<q}}function mJ(J,$){let Q=dJ(J),Z=HJ[$];if(Z===void 0)return $.startsWith("scale")?1:0;if($==="scale"){if(Q.values[Z]!==1)return Q.values[Z];let j=Q.values[9],q=Q.values[10];if(Math.abs(j-q)<0.000001&&j!==1)return j;return Q.values[Z]}if($==="rotate"){if(Q.values[Z]!==0)return Q.values[Z];let j=Q.values[7];if(j!==0&&Q.values[5]===0&&Q.values[6]===0)return j;return Q.values[Z]}return Q.values[Z]}function cJ(J,$){let Q=dJ(J),Z=HJ[$];if(Z===void 0)return"";return Q.units[Z]||uJ[Z]||""}function fJ(J){return J===0?"0":`${Math.round(J)}px`}function F6(J,$,Q){return`translate3d(${fJ(J)}, ${fJ($)}, ${fJ(Q)})`}function YJ(J,$,Q,Z){NJ.set(J,{x:$,y:Q,z:Z});let j=S0.get(J);if(j)j.pinDirty=!0}function P0(J){let $=NJ.has(J);if(NJ.delete(J),$){let Q=S0.get(J);if(Q)Q.pinDirty=!0}}function w(J){let $=S0.get(J),Q=NJ.get(J);if(!$){if(Q&&(Q.x!==0||Q.y!==0||Q.z!==0))return F6(Q.x,Q.y,Q.z);return""}if($.dirty===0&&!$.pinDirty&&$.cachedString)return $.cachedString;let{values:Z,units:j}=$;x.length=0;let q=$.used,z=Z[0];if(z!==0){let A=j[0];x.push(`perspective(${z}${A})`)}let G=Z[1],N=Z[2],H=Z[3],K=j[1],Y=j[2],_=j[3],X=q&2,U=q&4,W=q&8;if(G!==0||N!==0||H!==0||X||U||W){let A=K===Y&&(H===0&&!W||Y===_);if(H!==0||W)if(A)x.push(`translate3d(${G}${K}, ${N}${Y}, ${H}${_})`);else{if(G!==0||X)x.push(`translateX(${G}${K})`);if(N!==0||U)x.push(`translateY(${N}${Y})`);x.push(`translateZ(${H}${_})`)}else if(A&&(G!==0||X)&&(N!==0||U))x.push(`translate(${G}${K}, ${N}${Y})`);else{if(G!==0||X)x.push(`translateX(${G}${K})`);if(N!==0||U)x.push(`translateY(${N}${Y})`)}}if(Z[4]!==0||q&16)x.push(`rotate(${Z[4]}deg)`);if(Z[5]!==0||q&32)x.push(`rotateX(${Z[5]}deg)`);if(Z[6]!==0||q&64)x.push(`rotateY(${Z[6]}deg)`);if(Z[7]!==0||q&128)x.push(`rotateZ(${Z[7]}deg)`);let B=q&256,I=q&512,F=q&1024;if(Z[8]!==1||B)x.push(`scale(${Z[8]})`);else if(Z[9]!==1||Z[10]!==1||I||F)x.push(`scaleX(${Z[9]})`),x.push(`scaleY(${Z[10]})`);if(Z[11]!==1||q&2048)x.push(`scaleZ(${Z[11]})`);if(Z[12]!==0||q&4096)x.push(`skewX(${Z[12]}deg)`);if(Z[13]!==0||q&8192)x.push(`skewY(${Z[13]}deg)`);let L=x.join(" ");if(Q&&(Q.x!==0||Q.y!==0||Q.z!==0)){let A=F6(Q.x,Q.y,Q.z);$.cachedString=L?`${A} ${L}`:A}else $.cachedString=L;return $.dirty=0,$.pinDirty=!1,$.cachedString}function y(J,$=!1){if(S0.delete(J),$&&J instanceof HTMLElement)J.style.transform=""}var pJ=new Set,KJ=new Map,XJ=!1;function j0(J){pJ.add(J),XJ=!0}function y0(J,$,Q){let Z=KJ.get(J);if(!Z)Z=new Map,KJ.set(J,Z);Z.set($,Q),XJ=!0}function gJ(){if(!XJ)return;for(let J of pJ){let $=J;if($.style){let Q=w(J);$.style.transform=Q}}pJ.clear();for(let[J,$]of KJ){let Q=J;if(Q.style)for(let[Z,j]of $)try{if(Z.startsWith("--"))Q.style.setProperty(Z,j);else Q.style[Z]=j}catch{}}KJ.clear(),XJ=!1}var b0=!1;function V0(){return b0}function n0(J){if(b0){J();return}b0=!0;try{J(),gJ()}finally{b0=!1}}class T{static instance;listeners=[];_listenersDirty=!0;_listenersSnapshot=[];_isRunning=!1;rafId=null;lastTime=0;elapsedTime=0;lagThreshold=500;maxDelta=33;fpsFrames=0;fpsTime=0;currentFPS=60;lastFrameTime=0;constructor(){}static getInstance(){if(!T.instance)T.instance=new T;return T.instance}add(J,$=0){let Q=this.listeners.findIndex((Z)=>Z.callback===J);if(Q!==-1)this.listeners[Q].priority=$;else this.listeners.push({callback:J,priority:$});if(this.listeners.sort((Z,j)=>j.priority-Z.priority),this._listenersDirty=!0,!this._isRunning&&this.listeners.length>0)this.start()}remove(J){let $=this.listeners.findIndex((Q)=>Q.callback===J);if($!==-1)this.listeners.splice($,1),this._listenersDirty=!0;if(this.listeners.length===0&&this._isRunning)this.stop()}start(){if(this._isRunning)return;if(typeof requestAnimationFrame>"u"){this._isRunning=!1;return}this._isRunning=!0,this.lastTime=performance.now(),this.rafId=requestAnimationFrame(this.tick)}stop(){if(!this._isRunning)return;if(this._isRunning=!1,this.rafId!==null&&typeof cancelAnimationFrame<"u")cancelAnimationFrame(this.rafId),this.rafId=null}pause(){this.stop()}resume(){if(this.listeners.length>0)this.start()}tick=()=>{if(!this._isRunning)return;let J=performance.now(),$=J-this.lastTime;if($>this.lagThreshold)$=this.maxDelta,this.lastTime=J-$;else this.lastTime=J;let Q=$/1000;if(this.elapsedTime+=Q,this.lastFrameTime=$,this.updateFPS($),b0=!0,this._listenersDirty)this._listenersSnapshot=[...this.listeners],this._listenersDirty=!1;for(let Z of this._listenersSnapshot)try{Z.callback(Q,this.elapsedTime)}catch(j){console.error("[Motion] Uncaught error in animation callback:",j)}try{gJ()}catch(Z){console.error("[Motion] Uncaught error in render batch flush:",Z)}if(b0=!1,this._isRunning&&typeof requestAnimationFrame<"u")this.rafId=requestAnimationFrame(this.tick);else this.rafId=null};updateFPS(J){if(this.fpsFrames++,this.fpsTime+=J,this.fpsTime>=1000)this.currentFPS=Math.round(this.fpsFrames*1000/this.fpsTime),this.fpsFrames=0,this.fpsTime=0}getFPS(){return this.currentFPS}getLastDelta(){return this.lastFrameTime}isRunning(){return this._isRunning}isInTick(){return b0}getListenerCount(){return this.listeners.length}removeAll(){this.listeners=[],this.stop()}}var A6=new Set(["perspective","x","y","z","rotate","rotateX","rotateY","rotateZ","scale","scaleX","scaleY","scaleZ","skewX","skewY"]),L6=new Set(["perspective","x","y","z","width","height","top","left","right","bottom","margin","marginTop","marginRight","marginBottom","marginLeft","padding","paddingTop","paddingRight","paddingBottom","paddingLeft","borderRadius","fontSize","lineHeight"]),M6=new Set(["rotate","rotateX","rotateY","rotateZ","skewX","skewY"]),D6=new Set(["opacity","scale","scaleX","scaleY","scaleZ","z-index","zIndex"]),R6=new Set(["color","backgroundColor","borderColor","borderTopColor","borderRightColor","borderBottomColor","borderLeftColor","outlineColor","textDecorationColor","caretColor","fill","stroke"]),k6=new Set(["red","green","blue","yellow","orange","purple","pink","cyan","magenta","white","black","gray","grey","transparent","crimson","gold","silver","navy","teal","maroon","olive","lime","aqua","fuchsia","coral","salmon","tomato","indigo"]),oJ=new Set(["width","height"]);function C(J){return A6.has(J)}function q0(J){return R6.has(J)}function o(J){return J==="filter"}function W0(J){return J==="drawSVG"}function a0(J){return J==="path"}function r0(J){return J==="clip-path"||J==="clipPath"}function a(J){return J.startsWith("--")}function _J(J){if(typeof J!=="string")return!1;let $=J.trim().toLowerCase();if($.startsWith("#"))return!0;if(/^(rgb|rgba|hsl|hsla)\s*\(/.test($))return!0;let Q=O.color;if(Q)return Q.parseColor($)!==null;return k6.has($)}function S(J){if(typeof J==="number"){if(isNaN(J)||!isFinite(J))return console.warn(`[Motion] Property value is ${J}. This property will not animate correctly.`),{value:0,unit:""};return{value:J,unit:""}}if(typeof J==="string"){let $=J.match(/^([+\-*/])=(.+)/);if($){let Z=$[2].match(/([-+]?[\d.]+(?:e[-+]?\d+)?)([a-z%]*)/i);if(Z)return{value:parseFloat(Z[1]),unit:Z[2]||""}}let Q=J.match(/([-+]?[\d.]+(?:e[-+]?\d+)?)([a-z%]*)/i);if(Q)return{value:parseFloat(Q[1]),unit:Q[2]||""}}return{value:0,unit:""}}function m(J){if(D6.has(J))return"";if(M6.has(J))return"deg";if(L6.has(J))return"px";return""}function b6(J){if(J==="opacity")return 1;if(J.startsWith("scale"))return 1;return 0}function J0(J,$){if(!(J instanceof Element)){let Q=J;if($ in Q)return Q[$];return P7($)}if(C($))return mJ(J,$);try{if(typeof window<"u"&&window.getComputedStyle){let Z=window.getComputedStyle(J).getPropertyValue(c($));if(Z)return S(Z).value}}catch(Q){}return b6($)}function V6(J,$,Q,Z){if(!Z||Z==="px")return Q;if(typeof window>"u"||!window.getComputedStyle)return Q;let j=J,q=c($);try{let z=j.style.getPropertyValue(q);j.style.setProperty(q,`100${Z}`);let G=window.getComputedStyle(J),N=parseFloat(G.getPropertyValue(q));if(z)j.style.setProperty(q,z);else j.style.removeProperty(q);if(N&&isFinite(N)&&N!==0)return Q/N*100}catch{}return Q}function v0(J,$,Q){let Z=J0(J,$);if(J instanceof Element&&Q&&Q!=="px")return V6(J,$,Z,Q);return Z}function lJ(J,$){let Q=J,Z=c($);if(C($)){let j=Q.style.transform;y(J),Q.style.transform="";let q=mJ(J,$);return Q.style.transform=j,y(J),{value:q,unit:m($)}}try{if(typeof window<"u"&&window.getComputedStyle){let j=Q.style.getPropertyValue(Z);Q.style.removeProperty(Z);let z=window.getComputedStyle(J).getPropertyValue(Z);if(j)Q.style.setProperty(Z,j);if(z){let G=S(z);return{value:G.value,unit:G.unit}}}}catch(j){}return{value:b6($),unit:m($)}}function P7(J){if(J.startsWith("scale"))return 1;return 0}function c(J){return J.replace(/[A-Z]/g,($)=>`-${$.toLowerCase()}`)}function sJ(J){if(typeof J==="string")return/^[+\-*/]=/.test(J);return!1}function iJ(J){let $=J.match(/^([+\-*/])=/);return $?$[0]:null}function nJ(J,$,Q){switch(Q){case"+=":return J+$;case"-=":return J-$;case"*=":return J*$;case"/=":return $!==0?J/$:J;default:return $}}var z0=null,T0=null,y7=100,G0=new Map;function v7(J){let $=G0.get(J);if($!==void 0)G0.delete(J),G0.set(J,$);return $}function T7(J,$){if(G0.has(J))G0.delete(J);while(G0.size>=y7){let Q=G0.keys().next().value;if(Q!==void 0)G0.delete(Q);else break}G0.set(J,$)}function h7(J){return/^\s*[Mm]/.test(J)}function E6(){if(T0)return T0;if(typeof document>"u")throw Error("PathParser requires a browser environment");return z0=document.createElementNS("http://www.w3.org/2000/svg","svg"),z0.style.cssText="position:absolute;width:0;height:0;overflow:hidden;visibility:hidden;",z0.setAttribute("aria-hidden","true"),T0=document.createElementNS("http://www.w3.org/2000/svg","path"),z0.appendChild(T0),document.body.appendChild(z0),T0}function x6(J){if(J instanceof Element)return J.getAttribute("d");if(typeof J==="string"){if(h7(J))return J;if(typeof document<"u"){let $=document.querySelector(J);if($)return $.getAttribute("d")}}return null}function w6(J){let $=v7(J);if($!==void 0)return $;try{let Q=E6();Q.setAttribute("d",J);let Z=Q.getTotalLength();return T7(J,Z),Z}catch{return 0}}function aJ(J,$,Q=!1){try{let Z=E6();Z.setAttribute("d",J);let j=Z.getTotalLength(),q=Math.max(0,Math.min(1,$)),z=j*q,G=Z.getPointAtLength(z),N=0;if(Q)N=f7(Z,z,j);return{x:G.x,y:G.y,angle:N}}catch{return{x:0,y:0,angle:0}}}function f7(J,$,Q){let Z=Math.min(0.1,Q*0.001),j=Math.max(0,$-Z),q=Math.min(Q,$+Z),z=J.getPointAtLength(j),G=J.getPointAtLength(q),N=G.x-z.x,H=G.y-z.y;return Math.atan2(H,N)*(180/Math.PI)}function C6(J,$=[50,50]){if(!J)return{x:0,y:0};let Q=null;if(J instanceof Element)Q=J;else if(typeof J==="string"&&typeof document<"u")Q=document.querySelector(J);if(!Q||typeof Q.getBoundingClientRect!=="function")return{x:0,y:0};let Z=Q.getBoundingClientRect(),j=Z.width*$[0]/100,q=Z.height*$[1]/100;return{x:j,y:q}}function S6(J,$,Q){if(!J)return{x:0,y:0};let Z=null;if(J instanceof Element)Z=J;else if(typeof J==="string"&&typeof document<"u")Z=document.querySelector(J);if(!Z)return{x:0,y:0};let j=aJ($,0,!1);if(Q&&Q instanceof HTMLElement){let q=Q.style.transform,z=q&&q!=="none"&&q!=="";if(z)Q.style.transform="none";let G=Q.getBoundingClientRect(),N=Z.getBoundingClientRect();if(z)Q.style.transform=q;let H=N.left+N.width/2-(G.left+G.width/2),K=N.top+N.height/2-(G.top+G.height/2);return{x:H-j.x,y:K-j.y}}return{x:-j.x,y:-j.y}}function P6(){if(z0&&z0.parentNode)z0.parentNode.removeChild(z0);z0=null,T0=null,G0.clear()}function y6(J){return typeof J==="string"&&J.trim().toLowerCase()==="auto"}function u7(J,$){if(!oJ.has($))return 0;let Q=J,Z=c($);try{let j=Q.style.getPropertyValue(Z);Q.style.setProperty(Z,"auto");let q=$==="width"?Q.offsetWidth:Q.offsetHeight;if(j)Q.style.setProperty(Z,j);else Q.style.removeProperty(Z);return q}catch{return 0}}function d7(J,$,Q,Z){let j=O.color;if(!j)return null;let q=String(Q),z=q===""?j.getCurrentColor(J,$):j.parseColor(q,J),G=Z!==void 0?j.parseColor(String(Z),J):j.getCurrentColor(J,$);return{type:"color",property:$,isTransform:!1,startColor:G||new Float32Array([0,0,0,1]),endColor:z||new Float32Array([0,0,0,1])}}function m7(J,$,Q,Z){let j=O.filter;if(!j)return null;let q=String(Q),z=q===""?j.getCurrentFilter(J):j.parseFilter(q),G=Z!==void 0?j.parseFilter(String(Z)):j.getCurrentFilter(J);G=G||[],z=z||[];let N=j.mergeFilterArrays(G,z);return{type:"filter",property:$,isTransform:!1,startFilters:N.start,endFilters:N.end}}function c7(J,$,Q,Z){if(_J(Q)||Z!==void 0&&_J(Z)){let N=O.color;if(N){let H=String(Q),K=N.parseColor(H,J),Y=Z!==void 0?N.parseColor(String(Z),J):p7(J,$);return{type:"color",property:$,isTransform:!1,startColor:Y||new Float32Array([0,0,0,1]),endColor:K||new Float32Array([0,0,0,1])}}}let q,z,G;if(Z!==void 0&&Q!==void 0){let N=S(Z),H=S(Q);q=N.value,z=H.value,G=H.unit||N.unit}else if(Z!==void 0){let N=S(Z);q=N.value,z=v6(J,$),G=N.unit}else{let N=S(Q);q=v6(J,$),z=N.value,G=N.unit}return{type:"scalar",property:$,isTransform:!1,startValue:q,endValue:z,unit:G}}function p7(J,$){let Q=O.color;if(!Q)return null;try{if(typeof window<"u"&&window.getComputedStyle){let j=window.getComputedStyle(J).getPropertyValue($).trim();if(j)return Q.parseColor(j,J)}}catch{}return null}function v6(J,$){try{if(typeof window<"u"&&window.getComputedStyle){let Z=window.getComputedStyle(J).getPropertyValue($).trim();if(Z)return S(Z).value}}catch{}return 0}function g7(J,$,Q,Z){let j=O.drawSVG;if(!j)return null;let q=j.getPathLength(J),z=j.parseDrawSVG(Q,J)||{start:0,end:1},G=Z!==void 0?j.parseDrawSVG(Z,J)||{start:0,end:1}:j.getCurrentDrawSVG(J);return{type:"drawSVG",property:$,isTransform:!1,startDraw:G,endDraw:z,length:q}}function o7(J,$,Q,Z){let j=O.clipPath;if(!j)return null;let q;if(Q===void 0||Q==="")q=j.getCurrentClipPath(J);else q=j.parseClipPath(String(Q));let z;if(Z!==void 0)z=j.parseClipPath(String(Z));else z=j.getCurrentClipPath(J);if(!q)return null;if(!z)z=q;return{type:"clipPath",property:$,isTransform:!1,startClipPath:z,endClipPath:q}}function l7(J,$,Q,Z){let j=Q,q=Z,z=j?.target?j:q;if(!z?.target)return null;let G=x6(z.target);if(!G)return null;let N=w6(G);if(N===0)return null;let H,K;if(j?.target&&q?.target)H=q.start??0,K=j.end??1;else if(j?.target)H=j.start??0,K=j.end??1;else H=q.start??0,K=q.end??1;let Y=j?.rotate??q?.rotate??!1,_=j?.alignAt??q?.alignAt??[50,50],X=C6(J,_),U=j?.align??q?.align,W=S6(U,G,J);return{type:"path",property:$,isTransform:!1,pathData:G,pathLength:N,startProgress:H,endProgress:K,rotate:Y,alignOffset:X,pathOffset:W}}function BJ(J,$,Q,Z){let j=J instanceof Element;if(j&&a($))return c7(J,$,Q,Z);if(j&&q0($)){let _=d7(J,$,Q,Z);if(_)return _}if(j&&o($)){let _=m7(J,$,Q,Z);if(_)return _}if(j&&W0($)){let _=g7(J,$,Q,Z);if(_)return _}if(j&&r0($)){let _=o7(J,$,Q,Z);if(_)return _}if(j&&a0($)){let _=l7(J,$,Q,Z);if(_)return _}let q=y6(Q),z=Z!==void 0&&y6(Z);if(j&&(q||z)&&oJ.has($)){let _=u7(J,$);return BJ(J,$,q?_:Q,z?_:Z)}let G=C($),N=Z===void 0&&sJ(Q),H,K,Y;if(Z!==void 0){let _=S(Z);if(Q!==void 0){let X=S(Q);H=_.value,K=X.value,Y=X.unit||_.unit||m($)}else H=_.value,Y=_.unit||m($),K=v0(J,$,Y)}else if(N){let _=iJ(Q),X=J0(J,$),U=S(Q);H=X,K=nJ(X,U.value,_),Y=U.unit||m($)}else{let _=S(Q);Y=_.unit||m($),H=v0(J,$,Y),K=_.value}return{type:"scalar",property:$,isTransform:G,startValue:H,endValue:K,unit:Y}}function T6(){let J=new WeakMap;return{register($,Q){let Z=J.get($);if(!Z)Z=new Set,J.set($,Z);Z.add(Q)},registerMany($,Q){for(let Z of Q)this.register($,Z)},get($){return J.get($)},clearRegistry($){J.delete($)}}}var rJ=T6(),UJ=new WeakMap,tJ=T6();function h6(J){if(C(J))return["transform"];if(o(J))return["filter"];if(W0(J))return["strokeDasharray","strokeDashoffset"];return[J]}function s7(J,$){if(C($)&&!UJ.has(J)){if(J instanceof HTMLElement||J instanceof SVGElement){let Z=J.style.transform;UJ.set(J,Z||null)}}let Q=h6($);for(let Z of Q)rJ.register(J,Z)}function i7(J,$){for(let Q of $)s7(J,Q)}function n7(J){rJ.clearRegistry(J),UJ.delete(J)}function a7(J,$){if(!(J instanceof HTMLElement)&&!(J instanceof SVGElement))return;let Q=new Set;for(let Z of $)for(let j of h6(Z))Q.add(j);for(let Z of Q)if(a(Z))J.style.removeProperty(Z);else J.style[Z]=""}function r7(J){if(!(J instanceof HTMLElement)&&!(J instanceof SVGElement))return;let $=rJ.get(J);if(!$||$.size===0)return;for(let Q of $){if(Q==="transform"){let Z=UJ.get(J);if(Z!==void 0){if(Z)J.style.transform=Z;else J.style.removeProperty("transform");continue}}if(a(Q))J.style.removeProperty(Q);else J.style[Q]=""}}function t7(J){r7(J),n7(J)}function t0(J,$){tJ.registerMany(J,$)}function eJ(J){tJ.clearRegistry(J)}function e7(J){if(!(J instanceof HTMLElement)&&!(J instanceof SVGElement))return;let $=tJ.get(J);if(!$||$.size===0)return;for(let Q of $)J.style[Q]=""}function J9(J){e7(J),eJ(J)}O.styleReset={registerAnimatedProps:i7,clearAnimationStylesForProps:a7,clearAnimationStylesAndUnregister:t7,clearPinStylesAndUnregister:J9};class h{static _registry=new Map;static _users=new Map;static getOrCreate(J,$){let Q=h._registry.get($);if(!Q)Q=new h(J),h._registry.set($,Q);let Z=h._users.get($);if(!Z)Z=new Set,h._users.set($,Z);return Z.add(J),Q}static release(J,$){let Q=h._users.get($);if(!Q)return;if(Q.delete(J),Q.size===0){let Z=h._registry.get($);h._registry.delete($),h._users.delete($),Z?.cleanup()}}_id;_element=null;_originalStyles=null;_spacer=null;_currentState="before";_pinStart=0;_pinEnd=0;_pinDistance=0;_useFixedPin=!1;_pinSpacing;_elementHeight=0;_fixedTop=0;_fixedLeft=0;_width=0;_fixedBreakingAncestors=[];_isBodyPin=!1;_originalHtmlHeight="";_originalHtmlOverflow="";constructor(J){this._id=J}setup(J,$,Q,Z,j,q){if(typeof document>"u")return;this._isBodyPin=J===document.body,this._element=J,this._pinStart=$,this._pinEnd=Q;let z=this._pinDistance;this._pinDistance=Q-$,this._useFixedPin=j,this._pinSpacing=q;let G=this._isBodyPin?this._originalStyles!==null:this._spacer!==null;if(G&&this._currentState==="pinned"&&j)this._updateFixed("before");if(G&&this._spacer)this._spacer.style.width="";let N=G&&this._isBodyPin?document.documentElement.style.height:null;if(N!==null)document.documentElement.style.height=this._originalHtmlHeight;let H=J.getBoundingClientRect();if(N!==null)document.documentElement.style.height=N;let K=G&&this._spacer?window.getComputedStyle(this._spacer):window.getComputedStyle(J);if(this._elementHeight=H.height,!G)this._originalStyles={position:J.style.position,top:J.style.top,left:J.style.left,width:J.style.width,marginTop:J.style.marginTop,marginBottom:J.style.marginBottom,marginLeft:J.style.marginLeft,marginRight:J.style.marginRight,zIndex:J.style.zIndex};if(j)if(this._fixedTop=Z,this._isBodyPin){if(this._fixedLeft=H.left,this._width=H.width,!G)this._originalHtmlHeight=document.documentElement.style.height,this._originalHtmlOverflow=document.documentElement.style.overflow,this._detectFixedBreakingAncestors(J);document.documentElement.style.height=`${this._elementHeight+this._pinDistance}px`}else{let Y=this._resolvePinSpacing(),_=window.getComputedStyle(J),X=_.display,U=_.flexDirection,W=G&&this._spacer?.parentElement?this._spacer.parentElement:J.parentElement,B=W?/^(inline-)?flex$/.test(getComputedStyle(W).display):!1,I=/^(inline-)?flex$/.test(X),F=`
          display: ${X};
          ${I?`flex-direction: ${U};`:""}
        ...
        background: ${B};
        pointer-events: none;
      `;let L=document.createElement("span");return L.textContent=W,L.style.cssText=`
        position: absolute;
        ${I}: 0;
        top: 2px;
        background: ${B};
